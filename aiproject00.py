...
def index_to_excel_col(i: int) -> str:
    """0-based index -> Excel letters (0->A, 25->Z, 26->AA...)"""
    if i < 0:
...
def compute_groups(
    ...
    # Convert chosen MAP column to percentile if enabled
    ...
    # Build model features in exact order (aligns to weights)
    ...
    # Make numeric for non-map columns
    for c in feature_keys:
        if c != "map_percentile":
            work[c] = pd.to_numeric(work[c], errors="coerce")

    # Drop rows with missing values in ANY selected feature (no imputation)
    ...
    # Ranking proxy: sum_j w_j * z_ij (higher = better)
...
# Top title only (no extra sentence)
st.markdown(
...
# Select score columns by Excel letters (typing)
st.markdown("<div class='card'>", unsafe_allow_html=True)
st.markdown("<div class='sec'>Select score columns (Excel letters)</div>", unsafe_allow_html=True)
st.markdown(
    "<div class='tiny'>Examples: <code>AA AB</code> • <code>A:D, AA:AC</code> • <code>C, E, G</code></div>",
    unsafe_allow_html=True,
)
...
try:
    ...
    if idxs:
        ...
        for i in idxs:
            if i < 0 or i > max_idx:
                raise ValueError(f"Column out of range: {index_to_excel_col(i)} (file has {len(df_work.columns)} columns)")
...
st.markdown("</div>", unsafe_allow_html=True)import re
import streamlit as st
import pandas as pd
import numpy as np
